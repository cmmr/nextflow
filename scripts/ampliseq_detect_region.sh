...
# sheet with no fastq_2 column is single-end - and platform from the median read
# length, the same 1000-base line taxprofiler_samplesheet.sh draws. Together they
# give ampliseq's sequencing_type:
#
...
#            appended to ./notes.txt, and ./message.out on error
...
readonly MAX_PAIRS=2000
readonly MIN_READ_LENGTH=100
readonly PROBE_LENGTH=150
readonly MAX_AMBIGUOUS=3

# Above this median read length the run is long-read. No Illumina instrument
# reaches it, which is the same line taxprofiler_samplesheet.sh draws.
...
mapfile -t CHOSEN < <(awk -F'\t' -v max="$MAX_SAMPLES" -v layout="$READ_LAYOUT" '
    ...
    END {
        ...
        for (i = 0; i < n && taken < max; i += stride) {
            print fq1[i] "\t" fq2[i]
            taken++
        }
    }' "$SAMPLESHEET")
...
PAIRS_PER_SAMPLE=$(( (MAX_PAIRS + ${#CHOSEN[@]} - 1) / ${#CHOSEN[@]} ))

log "Sampling $PAIRS_PER_SAMPLE $READ_LAYOUT-end reads from each of ${#CHOSEN[@]} samples..."

# Turn the head of one FASTQ into FASTA probes named "<mate>_<sample>_<n>", so
# the hit table can be split by mate again, and record every read's full length
# for the platform call. Paired reads are cut to their 5' PROBE_LENGTH bases;
# single-end reads are kept whole, since their far end is the only account of
# where the amplicon stops. The subshell absorbs pigz being cut off when awk
# stops reading.
sample_reads() {
    ...
    { pigz -p "$THREADS" -dc "$file" || true; } | awk \
        -v mate="$mate" -v sample_index="$index" -v want="$PAIRS_PER_SAMPLE" \
        -v minlen="$MIN_READ_LENGTH" -v probe="$probe" -v maxn="$MAX_AMBIGUOUS" \
        -v lengths="$LENGTHS_TXT" '
        NR % 4 == 1 { n++; if (n > want) exit; next }
        NR % 4 == 2 {
            if (length($0) < minlen) next
            print length($0) >> lengths

            seq = toupper(probe ? substr($0, 1, probe) : $0)
            if (gsub(/N/, "N", seq) > maxn) next
            printf ">%s_%s_%s\n%s\n", mate, sample_index, n, seq
        }' >> "$READS_FASTA"
}
...
if [[ "$SAMPLED_READS" -lt "$MINIMUM_SAMPLED" ]]; then
    REASON="What this run sequenced could not be determined:"
    REASON+=" only $SAMPLED_READS reads were long enough to look at."
    fail "$REASON"
fi

# 3. The platform, from the median read length.
MEDIAN_LENGTH=$(sort -n "$LENGTHS_TXT" | awk '{ v[NR] = $1 } END { print v[int((NR + 1) / 2)] + 0 }')

if [[ "$MEDIAN_LENGTH" -gt "$LONG_READ_THRESHOLD" ]]; then
    READ_SPAN="long"
else
    READ_SPAN="short"
fi
...
if [[ "$READ_LAYOUT" == "paired" && "$READ_SPAN" == "long" ]]; then
    ...
    REASON+=" but their median length of $MEDIAN_LENGTH bases is longer than any"
    REASON+=" paired-end instrument produces."
    fail "$REASON"
fi
...
log "Reads are $READ_LAYOUT-end with a median length of $MEDIAN_LENGTH; calling them $SEQUENCING_TYPE."
...
VSEARCH_COMMAND+=" --id 0.60 --query_cov 0.80 --strand both"
...
MEASUREMENT=$(awk -F'\t' -v OFS='\t' '
...

    {
        ...
        if (!((target SUBSEP lo) in position) || !((target SUBSEP hi) in position)) next

        lo = position[target SUBSEP lo]
        hi = position[target SUBSEP hi]
...
report_too_few_aligned() {
    ...
    REASON+=" Only $ALIGNED_TOTAL of $SAMPLED_READS sampled reads look like 16S at all,"
    REASON+=" which usually means these are not 16S amplicons."
    fail "$REASON"
}
...

{
    ...
    printf '  Platform:   %s (%s-end, median read %s bases)\n' \
