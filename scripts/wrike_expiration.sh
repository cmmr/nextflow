...
# the author a warning mentions. The completion date the expired page is headed
# with, and the status, are in every task Wrike hands back - and asking for
# either by name is a 400, since only the fields Wrike calls optional may appear
# here.
list_dashboard_tasks() {
    ...
    while true; do
        args=(-G
            --data-urlencode 'fields=["customFields","authorIds"]'
